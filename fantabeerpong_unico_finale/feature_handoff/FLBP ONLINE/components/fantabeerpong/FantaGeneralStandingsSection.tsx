...
import { ArrowDown, ArrowUpDown, BarChart3, Search, Trophy, X } from 'lucide-react';
import { FANTA_GENERAL_STANDINGS_MOCK } from '../../services/fantabeerpong/mockData';
...
type SortField = 'rank' | 'totalPoints' | 'livePoints' | 'gapFromLeader';
...
const trendBadgeClass = (trend: FantaGeneralStandingsRow['trend']) => trend === 'up' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : trend === 'down' ? 'border-rose-200 bg-rose-50 text-rose-700' : 'border-slate-200 bg-slate-100 text-slate-600';
const trendLabel = (trend: FantaGeneralStandingsRow['trend']) => trend === 'up' ? 'In salita' : trend === 'down' ? 'In calo' : 'Stabile';

export const FantaGeneralStandingsSection: React.FC<Props> = ({ onOpenMyTeam, onOpenPlayers, onOpenTeamDetail }) => {
  const data = FANTA_GENERAL_STANDINGS_MOCK;
  ...
  const myTeam = React.useMemo(() => data.rows.find((row) => row.id === data.myTeamId) || null, [data]);
  ...
  const filteredRows = React.useMemo(() => {
    return data.rows
      ...
      .sort((left, right) => {
        const primary = sortField === 'rank' ? left.rank - right.rank : sortField === 'gapFromLeader' ? left.gapFromLeader - right.gapFromLeader : right[sortField] - left[sortField];
        if (primary !== 0) return primary;
        return left.teamName.localeCompare(right.teamName, 'it', { sensitivity: 'base' });
      });
  }, [data, myTeam, onlyMineWindow, searchTerm, sortField]);
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-[26px] border border-slate-200 bg-gradient-to-r from-slate-50 to-white p-5 shadow-sm md:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-beer-100 bg-beer-50 px-3 py-1 text-[11px] font-black uppercase tracking-[0.12em] text-beer-700"><BarChart3 className="h-3.5 w-3.5" />{data.editionLabel}</div>
            <div className="mt-3 text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">Classifica fantasy squadre</div>
            <div className="mt-2 text-sm font-semibold leading-6 text-slate-600">Tabella coerente con la leaderboard esistente dell’app, con ricerca, ordinamento e focus sulla tua posizione.</div>
          </div>
          ...
          <table className="w-full min-w-[980px] text-left text-sm">
            <thead className="bg-slate-50 text-xs font-black uppercase tracking-wider text-slate-500">
              <tr>
                ...
                <th className={`${thPad} ${stickyTh}`}>Squadra fantasy</th>
                ...
                <th className={`${thPad} ${stickyTh} text-center`}>Dif.</th>
                <SortTh field="gapFromLeader" label="Gap" />
                <th className={`${thPad} ${stickyTh} text-center`}>Trend</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {filteredRows.map((row) => (
                <tr key={row.id} className={row.isMine ? 'bg-beer-50/40' : 'bg-white'}>
                  ...
                  <td className={tdPad}>
                    <button type="button" onClick={() => onOpenTeamDetail?.(row.id)} className="w-full rounded-xl text-left transition hover:text-beer-700">
                      <div className="flex items-center gap-2">
                        <div className="min-w-0"><div className="truncate text-base font-black text-slate-950">{row.teamName}</div><div className="text-xs font-bold text-slate-500">{row.ownerLabel}</div></div>
                        {row.isMine && <span className="inline-flex rounded-full border border-beer-200 bg-beer-50 px-2.5 py-1 text-[11px] font-black uppercase tracking-wide text-beer-700">La tua</span>}
                      </div>
                    </button>
                  </td>
                  ...
                  <td className={`${tdPad} text-center font-black text-slate-900`}>{row.captainName}</td>
                  <td className={`${tdPad} text-center font-black text-slate-900`}>{row.defendersCount}</td>
                  <td className={`${tdPad} text-center font-black text-slate-900`}>{row.gapFromLeader === 0 ? 'Leader' : `-${row.gapFromLeader}`}</td>
      ...
      <div className={panelClass}><div className="text-xl font-black tracking-tight text-slate-950">Nota integrazione</div><div className="mt-2 text-sm font-semibold leading-6 text-slate-600">Il dettaglio della singola squadra fantasy è separato e torna sempre al percorso FantaBeerpong.</div></div>
    </div>
  );
};
