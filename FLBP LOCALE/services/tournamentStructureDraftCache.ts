...
export const buildTournamentStructureSourceSignature = (snapshot: TournamentStructureSnapshot): string => JSON.stringify({
  ...
  groups: (snapshot.tournament?.groups || []).map((group) => ({ id: group.id, teams: (group.teams || []).map((team) => team.id) })),
  matches: (snapshot.matches || []).map((match) => ({
    id: match.id, phase: match.phase || '', groupName: match.groupName || '', round: match.round || 0,
    teamAId: match.teamAId || '', teamBId: match.teamBId || '', status: match.status,
    scoreA: match.scoreA, scoreB: match.scoreB, hidden: !!match.hidden, isBye: !!match.isBye,
  })),
});
...
const writePointer = (entry: StoredTournamentStructureDraft): void => {
  try {
    localStorage.setItem(`${POINTER_PREFIX}:${entry.key}`, JSON.stringify({
      key: entry.key, workspaceId: entry.workspaceId, tournamentId: entry.tournamentId,
      sourceSignature: entry.sourceSignature, savedAt: entry.savedAt, storage: 'indexeddb',
    }));
  } catch { /* IndexedDB remains authoritative. */ }
};

export const writeTournamentStructureDraft = async (state: TournamentStructureDraftState, sourceSignature: string): Promise<boolean> => {
  const id = tournamentId(state.original); if (!id) return false;
  const entry: StoredTournamentStructureDraft = {
    key: draftKey(state.original), workspaceId: workspaceId(), tournamentId: id, sourceSignature,
    savedAt: new Date().toISOString(), original: state.original, present: state.present, log: state.log,
  };
  const db = await openDb(); if (!db) return false;
  try {
    const tx = db.transaction(STORE_NAME, 'readwrite'); tx.objectStore(STORE_NAME).put(entry);
    const committed = await transactionDone(tx); if (committed) writePointer(entry); return committed;
  } catch { return false; } finally { db.close(); }
};

export const readTournamentStructureDraft = async (source: TournamentStructureSnapshot, sourceSignature: string): Promise<TournamentStructureDraftState | null> => {
  if (!tournamentId(source)) return null;
  const db = await openDb(); if (!db) return null;
  try {
    const tx = db.transaction(STORE_NAME, 'readonly'); const request = tx.objectStore(STORE_NAME).get(draftKey(source));
    ...
    return { original: entry.original, present: entry.present, past: [], future: [], log: entry.log || [], lastResult: null };
  } catch { return null; } finally { db.close(); }
};

export const clearTournamentStructureDraft = async (snapshot: TournamentStructureSnapshot): Promise<void> => {
  if (!tournamentId(snapshot)) return; const key = draftKey(snapshot); const db = await openDb();
  if (db) {
    try { const tx = db.transaction(STORE_NAME, 'readwrite'); tx.objectStore(STORE_NAME).delete(key); await transactionDone(tx); }
    catch { /* retry on the next successful save/reset */ } finally { db.close(); }
  }
