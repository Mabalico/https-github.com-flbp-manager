...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ state, setState, onEnterTv }) => {
    ...
    const fantaPretournamentEnabled = state.fantaSettings?.enabled === true;
        ...
        return syncFantaPretournamentRosters(teams, { previousTeams: options?.previousTeams })
            .then((result) => {
                setFantaSyncFeedback({
                    tone: 'success',
                    message: `Fanta aggiornato: ${result.updated ?? 0} roster aggiornati${result.deferred ? `, ${result.deferred} sostituzioni in attesa` : ''}.`,
                });
            })
            .catch((error) => {
        ...
        >
            ...
            <header className="relative z-10 flex flex-row items-center justify-between gap-1.5 bg-white px-2.5 py-2 rounded-[18px] shadow-sm border border-slate-200 sm:gap-2 sm:px-4 sm:py-3 lg:rounded-[28px]">
                <div className="flex min-w-0 flex-1 items-center gap-1.5 sm:gap-2">
                    <h2 className="flex min-w-0 shrink-0 items-center gap-1 text-[12px] font-black leading-none text-slate-900 sm:gap-1.5 sm:text-base">
                        ...
                        <span className="max-w-[82px] truncate sm:max-w-none">{t('admin')}</span>
                    </h2>
                    
                    <div className="hidden sm:block w-px h-5 bg-slate-200 mx-0.5"></div>

                    <details ref={adminSectionMenuRef} className="relative group">
                        <summary className="list-none [&::-webkit-details-marker]:hidden inline-flex max-w-[112px] items-center gap-1 bg-slate-900 text-white px-2 py-1.5 rounded-xl text-[10px] font-black hover:bg-slate-800 transition-colors cursor-pointer select-none focus-visible:outline-none shadow-sm sm:max-w-none sm:gap-1.5 sm:px-3 sm:text-xs">
                            {adminSection === 'live' ? (
                                <>
                                    <PlayCircle className="w-3 h-3 shrink-0 sm:w-3.5 sm:h-3.5"/>
                                    <span className="sm:hidden">Live</span>
                                    <span className="hidden sm:inline">{t('admin_live_management')}</span>
                                </>
                            ) : adminSection === 'data' ? (
                                <>
                                    <Settings className="w-3 h-3 shrink-0 sm:w-3.5 sm:h-3.5"/>
                                    <span className="sm:hidden">Dati</span>
                                    <span className="hidden sm:inline">{t('admin_data_management')}</span>
                                </>
                            ) : (
                                <>
                                    <Brackets className="w-3 h-3 shrink-0 sm:w-3.5 sm:h-3.5"/>
                                    <span className="sm:hidden">Editor</span>
                                    <span className="hidden sm:inline">{t('admin_structural_editor')}</span>
                                </>
                            )}
                            <ChevronDown className="w-3 h-3 shrink-0 opacity-80 sm:w-3.5 sm:h-3.5" />
                        </summary>
            ...
            {tab === 'teams' && (
                <TeamsTabLazy
                    ...
                    state={state}
                    setState={setState}
                    fantaEnabled={fantaPretournamentEnabled}
