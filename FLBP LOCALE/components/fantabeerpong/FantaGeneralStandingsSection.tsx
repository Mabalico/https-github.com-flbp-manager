...
import { fetchFantaStandings, invalidateFantaConfigCache } from '../../services/fantabeerpong/fantaSupabaseService';
...
export const FantaGeneralStandingsSection: React.FC<Props> = ({ onOpenMyTeam, onOpenPlayers, onOpenTeamDetail }) => {
  ...
  return (
    <div className="space-y-5">
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center rounded-[22px] border border-slate-200 bg-white py-14 md:hidden">
        ...
        <div className="space-y-3 md:hidden">
          {filteredRows.map((row) => (
            <button
              key={row.id}
              type="button"
              onClick={() => onOpenTeamDetail?.(row.id)}
              className={`w-full rounded-[22px] border p-4 text-left shadow-sm transition active:scale-[0.99] ${row.isMine ? 'border-beer-200 bg-beer-50/70' : 'border-slate-200 bg-white'}`}
            >
                  ...
                  <tr key={row.id} className={row.isMine ? 'bg-beer-50/40' : 'bg-white hover:bg-slate-50/50 transition-colors'}>
                    ...
                    <td className={tdPad}>
                      <button type="button" onClick={() => onOpenTeamDetail?.(row.id)} className="w-full rounded-xl text-left transition hover:text-beer-700">
                        <div className="flex items-center gap-2">
                          <div className="min-w-0"><div className="truncate text-base font-black text-slate-950">{row.teamName}</div></div>
