...
| Snapshot pubblico live | polling selettivo live | server locale/LAN: 1s con ETag; sito Internet: 120–140s con jitter; pause in tab nascosta con refresh immediato al ritorno visibile | `App.tsx`, `services/supabasePublic.ts` | **SÌ** | Chiama `pullPublicWorkspaceLive()` su `public_workspace_live` con ETag/304. Il payload contiene solo `teams`, `tournament`, `tournamentMatches`, `fantaSettings`, `__schemaVersion`; se la migration manca, fallback al vecchio `public_workspace_state`. |
