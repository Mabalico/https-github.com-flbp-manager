...
| Reports | `components/admin/tabs/ReportsTab.tsx` | `imageProcessingService`, `simulationService`, `formInputUX` | aggiorna match score/stats, status (scheduled/playing/finished), OCR, simulazioni; campi statistici filtrati alle sole cifre senza input-wheel e `0` iniziale sovrascrivibile; il prossimo match non ricade mai su un referto già concluso |
