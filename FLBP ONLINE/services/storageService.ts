...
const LEGACY_TOURNAMENT_NAME_MAP: Record<string, string> = {
    'Pecora Nera': 'II Torneo Soci Pecora Nera',
};

export const normalizeLegacyTournamentName = (value?: string | null): string => {
    const trimmed = String(value || '').trim();
    if (!trimmed) return '';
    return LEGACY_TOURNAMENT_NAME_MAP[trimmed] || trimmed;
};
...
const normalizeTournamentDataName = (tournament: TournamentData | null): TournamentData | null => {
    if (!tournament) return null;
    return {
        ...tournament,
        name: normalizeLegacyTournamentName(tournament.name),
    };
};

const normalizeHallOfFameEntryName = (entry: HallOfFameEntry): HallOfFameEntry => ({
    ...entry,
    tournamentName: normalizeLegacyTournamentName(entry.tournamentName),
    sourceTournamentName: entry.sourceTournamentName
        ? normalizeLegacyTournamentName(entry.sourceTournamentName)
        : entry.sourceTournamentName,
});

const normalizeIntegrationScorerLabel = (entry: IntegrationScorerEntry): IntegrationScorerEntry => ({
    ...entry,
    source: entry.source ? normalizeLegacyTournamentName(entry.source) : entry.source,
    sourceLabel: entry.sourceLabel ? normalizeLegacyTournamentName(entry.sourceLabel) : entry.sourceLabel,
});
...
export const coerceAppState = (raw: any): AppState => {
    ...
    merged.tournament = normalizeTournamentDataName(
        merged.tournament && isObj(merged.tournament) ? (merged.tournament as TournamentData) : null
    );
...
export const buildTournamentAwards = (tournament: TournamentData, matches: Match[], teams: Team[]): HallOfFameEntry[] => {
    ...
    (matches || []).forEach(m => {
        ...
        m.stats.forEach(s => {
            ...
            if (!agg[key]) {
                agg[key] = { name: s.playerName, yob, birthDate, teamName: team?.name || undefined, points: 0, soffi: 0, games: 0 };
            }
            if (!agg[key].teamName && team?.name) agg[key].teamName = team.name;
...
export const setTournamentMvps = (
    ...
    ].find((row) => row.id === tournamentId) || null;
    const resolvedTournamentName = normalizeLegacyTournamentName(tournament?.name || tournamentName);
    const year = extractYearFromDateLike(tournament?.startDate || state.tournament?.startDate) || new Date().getFullYear().toString();
    ...
    const entries: HallOfFameEntry[] = Array.from(uniq.values()).map(p => ({
        ...
        type: 'mvp',
        teamName: (tournament?.teams || []).find((team) =>
            [team.player1, team.player2].filter(Boolean).some((name) => normalizeNameLower(name) === normalizeNameLower(p.name))
        )?.name,
        playerNames: [p.name],
        playerId: p.id,
        sourceType: 'archived_tournament',
        sourceTournamentId: tournamentId,
        sourceTournamentName: resolvedTournamentName,
