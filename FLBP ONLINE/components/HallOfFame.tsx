        ...
        <div className="space-y-3">
          <div className="space-y-3 sm:hidden">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-2xl bg-slate-950 text-xs font-black text-white">{idx + 1}</span>
                      <div className="min-w-0 truncate text-base font-black text-slate-950">{p.name}</div>
                    </div>
                  </div>
                  <div className="shrink-0 rounded-2xl bg-beer-50 px-3 py-2 text-right">
                    <div className="text-[10px] font-black uppercase text-beer-700">{t('titles_total')}</div>
                    <div className="text-xl font-black text-slate-950">{p.total}</div>
                  </div>
                </div>

                <div className="mt-4 grid grid-cols-2 gap-2 text-[11px] font-black uppercase tracking-wide text-slate-600">
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2">{t('tournament_winner')}: <span className="text-slate-950">{p.win}</span></div>
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2">{t('tournament_mvp')}: <span className="text-slate-950">{p.mvp}</span></div>
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2">{t('tournament_top_scorer')}: <span className="text-slate-950">{p.ts}</span></div>
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2">{t('tournament_defender')}: <span className="text-slate-950">{p.def}</span></div>
                  {(p.ts25 || p.def25) ? (
                    <div className="col-span-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2">
                      U25: <span className="text-slate-950">{t('tournament_top_scorer')} {p.ts25} · {t('tournament_defender')} {p.def25}</span>
                    </div>
                  ) : null}
                </div>
              </button>
            ...
            {searchedPlayers.length === 0 && (
              <div className="rounded-[22px] border border-slate-200 bg-white p-8 text-center text-slate-400">
                {t('no_hof_records')}
              </div>
            )}
          </div>

          <div className="hidden bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden sm:block">
