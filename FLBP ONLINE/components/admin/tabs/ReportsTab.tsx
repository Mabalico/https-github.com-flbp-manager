                            ...
                            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3">
                                <div className="flex items-center justify-between gap-3 flex-wrap">
                                    <div>
                                        ...
                                        <div className="text-[11px] font-bold text-slate-500 mt-1">
                                            Scegli il match, compila i dati e salva il referto.
                                        </div>
                                    </div>
                                    <div className="text-[11px] font-bold text-slate-500">
                                        {matchesForSelect.length}/{msAll.length} mostrati
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-wrap gap-2 text-[11px] font-bold text-slate-500">
                                    <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-slate-700">
                                        In corso: <span className="font-mono">{playing?.code || '-'}</span>
                                    </span>
                                    <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-slate-700">
                                        Prossimo: <span className="font-mono">{current?.code || '-'}</span>
                                    </span>
                                    <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-slate-700">
                                        Flusso: match {selected ? 'selezionato' : 'da aprire'}
                                    </span>
                                </div>
                            </div>
                                    ...
                                    <div className="border border-slate-200 rounded-xl p-4 space-y-4 bg-white">
                                        <div className="flex items-center justify-between gap-2 flex-wrap">
                                            <div className="font-black text-slate-900">
                                                ...
                                                {selected.isTieBreak && (
                                                    <span className={`text-[10px] font-black px-2 py-0.5 rounded-full border shrink-0 ${selectedTeams.length >= 3 ? 'bg-amber-100 text-amber-800 border-amber-200' : 'bg-amber-50 text-amber-800 border-amber-200'}`}>
                                                        SPAREGGIO{selectedTeams.length >= 3 ? ' MULTI' : ''}{typeof selected.targetScore === 'number' ? ` a ${selected.targetScore}` : ''}
                                                    </span>
                                                )}
                                            ...
                                            <div className="text-xs font-bold text-slate-500 uppercase">
                                                {selected.phase === 'groups' ? 'GIRONI' : 'BRACKET'} • {selected.status}
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2 flex-wrap">
                                            ...
                                            <div className="text-[10px] font-bold text-slate-400">
                                                Suggerimento: salva “Giocata” per propagare il vincitore nel tabellone.
                                            </div>
                                        </div>
                                            ...
                                            {!isMulti ? (
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                    <div className="bg-slate-50 border border-slate-200 rounded-xl p-3">
                                                        <div className="font-black text-xs text-slate-700 mb-2 uppercase">{tA?.name || getTeamName(selected.teamAId)}</div>
                                                        {tA?.id && tA.id !== 'BYE' && (
                                                            <>
                                                    ...
                                                    <div className="bg-slate-50 border border-slate-200 rounded-xl p-3">
                                                        <div className="font-black text-xs text-slate-700 mb-2 uppercase">{tB?.name || getTeamName(selected.teamBId)}</div>
                                                        {tB?.id && tB.id !== 'BYE' && (
                                                            <>
