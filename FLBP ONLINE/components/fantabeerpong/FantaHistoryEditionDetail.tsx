  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-40">
        ...
        <p className="mt-4 text-sm font-black uppercase tracking-widest text-slate-500">{t('fanta_history_detail_loading')}</p>
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className="py-20 text-center animate-fade-in">
        ...
        <p className="mt-4 font-bold italic text-slate-500">{t('fanta_history_detail_not_found')}</p>
        <button onClick={onBack} className="mt-4 text-xs font-black uppercase tracking-widest text-beer-600 underline underline-offset-4">{t('fanta_history_detail_back')}</button>
      </div>
    );
  }
        ...
        <div className="rounded-[30px] border border-slate-200 bg-gradient-to-r from-slate-50 to-white p-5 shadow-sm md:p-7">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm font-semibold leading-6 text-slate-600">Classifica completa dell'edizione Fanta archiviata.</p>
            </div>
            <button type="button" onClick={() => setDetailView('overview')} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-black text-slate-700 shadow-sm transition hover:bg-slate-50">
              <ArrowLeft className="h-4 w-4" />
              Torna all'edizione
        ...
        <div className={panelClass}>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm font-bold leading-6 text-slate-500">Clicca una squadra per aprire il dettaglio della rosa e dei punti.</p>
            </div>
            <div className="text-sm font-black uppercase tracking-wide text-slate-500">{data.standings.length} squadre</div>
          </div>
              ...
              <button key={row.teamId} type="button" onClick={() => openTeamDetail(row.teamId)} className={`w-full rounded-[22px] border px-4 py-4 text-left transition hover:-translate-y-0.5 hover:shadow-md ${row.rank === 1 ? 'border-amber-200 bg-amber-50/70' : 'border-slate-200 bg-slate-50'}`}>
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 text-sm font-bold uppercase tracking-tight text-slate-500">{row.totalPoints} {t('fanta_points_label')}</div>
                  </div>
