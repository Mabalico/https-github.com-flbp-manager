...
-- Player-centric view for Fanta Player Standings
create or replace view fanta_player_standings as
select 
  ms.player_key,
  ms.player_name,
  max(ms.team_name) as real_team_name,
  sum(ms.canestri) as live_points,
  sum(ms.soffi) as blows,
  (
    select count(distinct team_id) 
    from fanta_rosters 
    where player_id = ms.player_key
  ) as selected_by_teams
from tournament_match_stats ms
group by ms.player_key, ms.player_name;
