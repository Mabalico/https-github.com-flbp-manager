...
-- Only server-controlled claims are accepted; user_metadata is never authorization.
...
-- ===== BEGIN supabase\migrations\20260417000400_fanta_lock_only_after_first_match.sql =====
...
-- ===== BEGIN supabase\migrations\20260623000100_fanta_roster_stat_key_fallback.sql =====
...
-- ===== END supabase\migrations\20260623000100_fanta_roster_stat_key_fallback.sql =====


-- ===== BEGIN supabase\migrations\20260623000200_referee_auth_rate_limit.sql =====
...
-- ===== END supabase\migrations\20260623000200_referee_auth_rate_limit.sql =====

-- ===== BEGIN supabase\migrations\20260629000100_fanta_save_team_public_live_fallback.sql =====
-- Keep Fanta roster saves aligned with the public live mirror.
--
-- The web client resolves the active Fanta tournament from public_tournaments.
-- If normalized private tables lag behind, the old RPC could reject a valid
-- live tournament. Resolve requested/config/latest live tournaments from both
-- layers and materialize the public tournament row into tournaments when needed,
-- so the existing fanta_teams FK remains valid.
...
begin
  ...
  -- If the public mirror is ahead of normalized tables, keep the parent FK row
  -- available without clearing any existing Fanta teams.
  insert into tournaments (workspace_id, id, name, start_date, type, config, is_manual, status, updated_at)
  ...
  select coalesce(
    (select t.config from tournaments t where t.workspace_id = p_workspace_id and t.id = v_tournament_id limit 1),
    (select p.config from public_tournaments p where p.workspace_id = p_workspace_id and p.id = v_tournament_id limit 1),
    '{}'::jsonb
  )
  ...
  select (
    ...
    or exists (
      select 1 from public_tournament_matches
      where workspace_id = p_workspace_id
        and tournament_id = v_tournament_id
        and hidden = false
        and is_bye = false
...
grant execute on function public.fanta_save_team(text, text, text, jsonb) to authenticated;
-- ===== END supabase\migrations\20260629000100_fanta_save_team_public_live_fallback.sql =====

-- ===== BEGIN supabase\migrations\20260630000100_fanta_archive_snapshots.sql =====
...
-- ===== END supabase\migrations\20260630000100_fanta_archive_snapshots.sql =====

-- ===== BEGIN supabase\migrations\20260701000300_fanta_protect_pretournament_container.sql =====
...
-- ===== END supabase\migrations\20260701000300_fanta_protect_pretournament_container.sql =====

-- ===== BEGIN supabase\migrations\20260702000100_fanta_promote_pretournament_config.sql =====
...
-- ===== END supabase\migrations\20260702000100_fanta_promote_pretournament_config.sql =====

-- ===== BEGIN supabase\migrations\20260702000200_fanta_team_owner_labels.sql =====
...
-- ===== END supabase\migrations\20260702000200_fanta_team_owner_labels.sql =====


-- ===== BEGIN supabase/migrations/20260704000100_fanta_award_refs_source_and_winner_team.sql =====
...
-- ===== BEGIN supabase/migrations/20260924000100_admin_auth_trusted_claims.sql =====
...
$$;
-- ===== END supabase/migrations/20260924000100_admin_auth_trusted_claims.sql =====
