...
const suiteFile=path.join(root,'supabase/tests/database_restore_schema.sql');
...
if(databaseUrl){
  ...
  try{
    ...
    console.log(`Applied all ${files.length} ${legacy?'LOCALE':'ONLINE'} migrations to in-memory PostgreSQL.`);
    if(legacy){
      const actor='00000000-0000-4000-8000-000000000091';
      await db.exec(`
        insert into auth.users(id,email) values ('${actor}','legacy-restore@test.invalid');
        insert into public.admin_users(user_id,email) values ('${actor}','legacy-restore@test.invalid');
        insert into public.workspaces(id) values ('restore-test-legacy');
        select set_config('request.jwt.claims','{"role":"service_role"}',false);
      `);
      const backup=(await db.query("select public.flbp_export_application_database('restore-test-legacy') as backup")).rows[0].backup;
      await assert.rejects(db.query('select public.flbp_restore_application_database($1,$2::jsonb,$3::uuid,$4,null)',
        ['restore-test-legacy',JSON.stringify(backup),actor,'legacy-denied']),/Aggiornare lo schema ONLINE/);
      assert.equal((await db.query("select count(*)::integer as count from public.database_restore_checkpoints where workspace_id='restore-test-legacy'")).rows[0].count,0);
      console.log('PASS: complete legacy schema accepts the migration/export but refuses unsafe restore before creating a checkpoint.');
    }else{
      ...
      console.log('Full-schema restore integration suite passed; all fixtures rolled back.');
    }
  }catch(error){
    console.error(`Full-schema restore test failed: ${error.message}`);
