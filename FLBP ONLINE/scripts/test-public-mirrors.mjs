...
const suiteFile=path.join(root,'supabase/tests/public_mirrors.sql');
...
if(databaseUrl){
  ...
  try{
    ...
    const files=(await fs.readdir(migrationDir)).filter(name=>name.endsWith('.sql') && (!legacy || name<'20260701000000' || name.startsWith('20260924000100') || name.startsWith('20260924000200') || name.startsWith('20260924000300') || name.startsWith('20260924000400'))).sort();
    ...
    console.log(`Applied ${files.length} ${legacy?'LOCALE migrations through June plus additive September 001-004':'ONLINE migrations (complete sequence)'} to in-memory PostgreSQL.`);
    const hardening=await fs.readFile(path.join(migrationDir,'20260924000400_authoritative_public_mirrors.sql'),'utf8');
    await db.exec(hardening);
      ...
      console.log('Public mirror integration suite passed; all fixtures rolled back.');
      if(!legacy){
        await db.exec(await fs.readFile(path.join(root,'supabase/tests/deployed_stabilization_readonly.sql'),'utf8'));
        console.log('PASS: deployed-stabilization read-only metadata probe on the complete schema');
      }
  }catch(error){
    console.error(`Public mirror test failed: ${error.message}`);
