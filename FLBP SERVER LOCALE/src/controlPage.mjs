...
<body><main class="box">
  ...
  <div class="row">
    ...
    <button id="reconcile" class="alt">Risolvi transizione</button>
...
const token=async()=>{const cached=sessionStorage.getItem('flbp_local_control_token');if(cached)return cached;try{const res=await fetch('/control/local-session',{method:'POST',headers:{'content-type':'application/json'},body:'{}'});if(res.ok){const body=await res.json();if(body.token){setToken(body.token);return body.token}}}catch(error){}return prompt('Token Admin del server locale:')||''};
const api=async(path,options={})=>{const t=await token();setToken(t);const requestOptions=Object.assign({},options,{headers:Object.assign({'content-type':'application/json','x-flbp-local-token':t},options.headers||{})});const res=await fetch(path,requestOptions);const body=await res.json().catch(()=>({error:'Risposta non valida'}));if(!res.ok)throw new Error(body.error||('HTTP '+res.status));return body};
const refresh=async()=>{try{const s=await fetch('/health').then(r=>r.json());const transitioning=s.transition&&s.transition!=='idle';const restored=s.transition==='restore-pending';const stateLabel=restored?'RIPRISTINO BLOCCATO':(transitioning?'DRAINING / VERIFICA RICHIESTA':(s.active?'PRIMARIO LOCALE':'STANDBY'));document.querySelector('#reconcile').style.display=transitioning?'inline-block':'none';const writerLease=s.adminWriterLease||{};const lease=writerLease.active?('in uso da '+(writerLease.holderLabel||'una finestra Admin')):'libero';const version=s.version===null||typeof s.version==='undefined'?'—':s.version;document.querySelector('#status').innerHTML=[['Stato',stateLabel],['Transizione',s.transition||'idle'],['Versione',version],['Operazioni in coda',s.pendingOperations],['Autorità Admin',s.active?'SQLite locale':'Supabase'],['Letture pubbliche','Mirror Supabase (online) / SQLite (LAN)'],['Lease Admin',lease],['Ultimo backup cloud',s.lastBackupAt||'—'],['Replica secondaria',s.lastSecondaryBackupAt?(s.lastSecondaryBackupAt+' · v'+s.lastSecondaryBackupVersion):'non configurata/eseguita'],['Aggiornato',s.updatedAt||'—'],['Epoch',s.primaryEpoch||'—']].map(([a,b])=>'<div class="card"><div class="label">'+a+'</div><div class="value '+(a==='Stato'?(transitioning?'standby':(s.active?'ok':'standby')):'')+'">'+b+'</div></div>').join('');msg.className=transitioning?'msg bad':'msg';msg.textContent=(s.configErrors||[]).length?'Configurazione da completare:\\n'+s.configErrors.join('\\n'):(transitioning?'Scritture bloccate in sicurezza. Premi “Risolvi transizione”: Supabase verificherà automaticamente nodo, epoch e ultima operazione.':'Server pronto.')}catch(e){msg.className='msg bad';msg.textContent=e.message}};
...
document.querySelector('#activate').onclick=action('/control/activate');document.querySelector('#reconcile').onclick=action('/control/reconcile');document.querySelector('#backup').onclick=action('/control/backup');document.querySelector('#deactivate').onclick=action('/control/deactivate');refresh();setInterval(refresh,5000);
`;
